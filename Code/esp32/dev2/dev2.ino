...
void sendFlameToFirebase() {
  ...
  if (Firebase.RTDB.setJSON(&fbdo2,
      "/devices/device_02/sensors/flame", &json)) {
...
void setup() {
  ...
  Serial.println(F("========================================"));
  Serial.println(F("  ESP32 Forest Fire [device_02] + GPS"));
  Serial.println(F("========================================"));
...
void loop() {
  ...
  if (fireConfirmed && !lastFireState) {
    ...
    Firebase.RTDB.setJSON(&fbdo,
      "/devices/device_02/alerts", &aJson);
  }
